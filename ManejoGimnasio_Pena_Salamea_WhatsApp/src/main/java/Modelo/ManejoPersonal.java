...
public class ManejoPersonal implements Serializable {
    ...
    public ArrayList<Personal> buscar(String criterio, String valor) {
        ArrayList<Personal> resultados = new ArrayList<>();
        for (Personal empleado : personal) {
            switch (criterio) {
                case "Cedula":
                    if (empleado.getCedula().equalsIgnoreCase(valor)) {
                        resultados.add(empleado);
                    }
                    break;
                case "Nombre":
                    if (empleado.getNombres().equalsIgnoreCase(valor)) {
                        resultados.add(empleado);
                    }
                    break;
                case "Apellido":
                    if (empleado.getApellidos().equalsIgnoreCase(valor)) {
                        resultados.add(empleado);
                    }
                    break;
                case "Rol":
                    if (empleado.getRol().equalsIgnoreCase(valor)) {
                        resultados.add(empleado);
                    }
                    break;
                case "Jornada":
                    if (empleado.getJornada().toString().equalsIgnoreCase(valor)) {
                        resultados.add(empleado);
                    }
                    break;
                default:
